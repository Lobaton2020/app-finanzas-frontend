...
import { loadMovementEgress, loadMovementIngress } from "@app/movements/state/movement.action";
...
export class MovementListComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.dataSource = new MatTableDataSource<MovementType>(this.movementList?.items || [])
  ...
  onPaginatorChange($event: PageEvent) {
    ...
    if (this.selectControl.toLowerCase().includes(TYPE_EGRESS.toLowerCase())) {
      return this.store.dispatch(loadMovementEgress({ selectControl: TYPE_EGRESS, page: $event.pageIndex, limit: $event.pageSize }));
    }
    return this.store.dispatch(loadMovementIngress({ selectControl: TYPE_INGRESS, page: $event.pageIndex, limit: $event.pageSize }))
